    ...
    return (
        <nav className={`fixed top-0 left-0 w-full z-50 transition-all duration-500 ease-in-out ${scrolled ? 'py-4 translate-y-0' : 'py-10'}`}>
            <div className="max-w-7xl mx-auto px-6">
                <div className={`premium-blur rounded-full px-10 py-5 flex items-center justify-between transition-all duration-500 ${
                    scrolled ? 'border-white/10 shadow-2xl scale-[0.98]' : 'border-white/5 shadow-none'
                } relative overflow-hidden group`}>
                    <div className="absolute inset-0 bg-brand-blue/5 -z-10 group-hover:bg-brand-blue/10 transition-colors"></div>

                    {/* Logo / Name */}
                    <div className="text-2xl font-black font-outfit text-white tracking-widest uppercase">
                        K<span className="text-brand-blue">.</span>KM
                    </div>
                        ...
                        <div className="premium-blur rounded-[3rem] p-10 flex flex-col gap-8 border border-white/10 shadow-3xl">
                            {navLinks.map((link) => (
                                <a
                                    key={link.name}
                                    href={link.href}
                                    className="text-3xl font-black uppercase tracking-tighter text-white/60 hover:text-white transition-colors !no-underline"
